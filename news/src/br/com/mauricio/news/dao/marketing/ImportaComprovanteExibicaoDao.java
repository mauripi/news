...
*/
public class ImportaComprovanteExibicaoDao {
	...
	public List<MidiaMais> importarMidiaMais(String excelFilePath) throws EncryptedDocumentException, InvalidFormatException, IOException{
       ...
       for(int i=8;i<firstSheet.getLastRowNum();i++){
           ...
           if(!getCellValue(aCell).equals("CLIENTE")&&!getCellValue(aCell).toString().contains("TOTAL DE INSERÇÕES:")){
	           ...
	           m.setHorario(getCellValue(gCell)+"");            
	           ...
	           if(isDoubleTry(getCellValue(iCell).toString()))
	        	   m.setAudiencia(Double.parseDouble(getCellValue(iCell).toString()));            
	           else
