...
public class NotaDebitoEnergiaDao implements Serializable {
	...
	public List<ViewNotaDebito> getNotaDebitoConsolidados(int ano){
		String sql ="SELECT * FROM (SELECT p.nome,p.tiploc, "
				...
				+ "END [mes] ,valor FROM notadebitopraca d, mengloc p WHERE (d.mengloc_id=p.id) and [ano]= :ano) "
		...
		for(Object[] o:list){
			...
			v.setTiploc(Integer.parseInt(o[1].toString()));
			
			if(o[2]==null)
				v.setJaneiro(new Double("0.0"));
			else
				v.setJaneiro(new Double(o[2].toString()));				
			
			if(o[3]==null)
				v.setFevereiro(new Double("0.0"));
			else
				v.setFevereiro(new Double(o[3].toString()));
							
			if(o[4]==null)
				v.setMarco(new Double("0.0"));
			else
				v.setMarco(new Double(o[4].toString()));			
			
			if(o[5]==null)
				v.setAbril(new Double("0.0"));
			else
				v.setAbril(new Double(o[5].toString()));			
			
			if(o[6]==null)
				v.setMaio(new Double("0.0"));
			else
				v.setMaio(new Double(o[6].toString()));			
			
			if(o[7]==null)
				v.setJunho(new Double("0.0"));
			else
				v.setJunho(new Double(o[7].toString()));			

			if(o[8]==null)
				v.setJulho(new Double("0.0"));
			else
				v.setJulho(new Double(o[8].toString()));			
			
			if(o[9]==null)
				v.setAgosto(new Double("0.0"));
			else
				v.setAgosto(new Double(o[9].toString()));										
			
			if(o[10]==null)
				v.setSetembro(new Double("0.0"));
			else
				v.setSetembro(new Double(o[10].toString()));			
			
			if(o[11]==null)
				v.setOutubro(new Double("0.0"));
			else
				v.setOutubro(new Double(o[11].toString()));			
			
			if(o[12]==null)
				v.setNovembro(new Double("0.0"));
			else
				v.setNovembro(new Double(o[12].toString()));			
			
			if(o[13]==null)
				v.setDezembro(new Double("0.0"));
			else
				v.setDezembro(new Double(o[13].toString()));			
